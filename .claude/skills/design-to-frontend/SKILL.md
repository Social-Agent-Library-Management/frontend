...
| 팀원 | 타입 | 역할 | 스킬 | 출력 |
|------|------|------|------|------|
| design-interpreter | 커스텀(sonnet) | 디자인→구조화 스펙 | design-spec-extraction | `_workspace/01_design-spec.md` |
...
| qa-inspector | general-purpose(sonnet) | 검증·정합성·중복 검사 | frontend-qa-verification | `_workspace/04_qa-report.md` |
| (리더 = 오케스트레이터) | — | 조율·통합·사용자 보고 | — | 최종 요약 |

> **모델 배정(혼합):** 추론·구현 핵심인 **architect·implementer는 opus**, 해석·검증인 **interpreter·qa는 sonnet**. 각 에이전트 frontmatter에도 동일하게 고정돼 있고, 아래 TeamCreate의 스폰 `model`이 최종 결정한다(스폰 override가 frontmatter보다 우선). 자동 다운그레이드는 없으므로 두 곳을 일치시킨다. 특정 실행만 바꾸려면 스폰 `model`만 조정하면 된다.
...
TeamCreate(
  ...
  members: [
    { name: "design-interpreter", agent_type: "design-interpreter", model: "sonnet",
...
3. **문서 동기화 확인:** 신규 재사용 컴포넌트가 `src/components/README.md`의 "컴포넌트 인벤토리" 표에 반영됐는지 확인(누락 시 반영). 이 README 변경은 커밋에 포함한다. CLAUDE.md 변경 이력은 **하네스 자체가 바뀔 때만** 갱신(기능 작업은 기록하지 않음).
